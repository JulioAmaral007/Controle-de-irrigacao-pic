...
void main()
{
    ...
    while(1)
    {
        ...
        if(turnON==1)
        {
            do { unsigned char cmd = 0x08; if ( 1 == 1 ) { cmd |= 1 << 2; } if ( 1 == 1 ) { cmd |= 1 << 1; } if ( 1 == 1 ) { cmd |= 1; } do { LCD_Write( (cmd & 0xF0) >> 4 ); LCD_Write( cmd & 0x0F); } while ( 0 ); } while ( 0 );;
        }
        else
        {
            do { unsigned char cmd = 0x08; if ( 0 == 1 ) { cmd |= 1 << 2; } if ( 0 == 1 ) { cmd |= 1 << 1; } if ( 0 == 1 ) { cmd |= 1; } do { LCD_Write( (cmd & 0xF0) >> 4 ); LCD_Write( cmd & 0x0F); } while ( 0 ); } while ( 0 );;
        }
    }
}
